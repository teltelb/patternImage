// PatternImageTool.tsx
// フォトショップ風の企業向けUIと機能を備えた画像パターン生成ツール
...
const PatternImageTool: React.FC = () => {
  ...
  const generateRandomRotations = (): number[][] => {
    return Array.from({ length: rows }, () => Array.from({ length: cols }, () => getRandomRotation()));
  };
  ...
  const savePreset = () => {
    ...
    setPresets(updated);
    localStorage.setItem("pattern_presets", JSON.stringify(updated.filter(p => !defaultPresets.some(dp => JSON.stringify(dp) === JSON.stringify(p)))));
  };
        ...
        <div className="button-group">
          <select value={selectedPresetIndex ?? ""} onChange={(e) => {
            ...
            {presets.map((preset, index) => (
              <option key={index} value={index}>{preset[0]}×{preset[1]} / {preset[2]}×{preset[3]} {index < defaultPresets.length ? "(標準)" : "(カスタム)"}</option>
            ))}
          </select>
          ...
          <Layer>
            ...
            {[...Array(rows)].map((_, row) =>
              [...Array(cols)].map((_, col) => {
                ...
                const shouldDraw = (isEvenCol && isOddRow) || (isOddCol && !isOddRow);
                if (!shouldDraw || imageList.length === 0) return null;
                let img = imageList.length === 1 ? imageList[0] : imageList[row % imageList.length];
                if (!img) return null;
